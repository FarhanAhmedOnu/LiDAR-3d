...
class Lidar3DScanner(Node):
    def __init__(self):
        ...
        self.baud_rate = 115200  # CHANGED: Updated to match servo.ino 
        self.ser = None
    ...
    def connect_and_test_servo(self):
        try:
            ...
            # Flush any startup messages (like "READY" from servo.ino source: 2)
            self.ser.reset_input_buffer()

            # Send Test Command
            print("[System] Testing servo connection...")
            # CHANGED: Added 'ANGLE:' prefix to match servo.ino 
            ...
            while time.time() - start < 3.0:
                if self.ser.in_waiting:
                    ...
                    # CHANGED: Check for 'ACK_ANGLE:' to match servo.ino 
    ...
    def move_servo(self, angle):
        ...
        # CHANGED: Formatted command with 'ANGLE:' prefix 
    ...
    def polar_to_cartesian_3d(self, ranges, angle_min, angle_inc, servo_angle_deg):
        # Convert servo angle to radians (0 deg is vertical, so we subtract 90)
        ...
        for r in ranges:
            if 0.05 < r < 4.0: 
                # Standard 2D polar to cartesian
            ...
            while self.is_running:
                for angle in sweep_range:
                    ...
                    if self.move_servo(angle):
                        # Clear old event, wait for new scan
    ...
    def cleanup(self):
        ...
        self.is_running = False
        if self.ser:
            # CHANGED: Return to 90 using correct syntax
            self.ser.write(b"ANGLE:90\n")
            self.ser.close()
        self.stop_lidar_driver()
        self.destroy_node()
